...
def prever_crescimento_total(df: pd.DataFrame, tipo: str = "TotalSizeMB", salvar_excel=True):
    ...
    # Gerar previsão para os próximos 365 dias
    ...
    # Adiciona coluna em GB se tipo for TotalSizeMB
    if tipo == "TotalSizeMB":
        previsao_mensal[f"Previsao_{tipo}_Mensal_GB"] = (
            previsao_mensal[f"Previsao_{tipo}_Mensal"] / 1024
        ).round(2)
    ...
    if salvar_excel:
        ...
        with pd.ExcelWriter(caminho_saida, engine="openpyxl") as writer:
            ...
            # Formatar MB (coluna B)
            for row in worksheet.iter_rows(min_row=2, min_col=2, max_col=2):
                for cell in row:
                    cell.number_format = '#,##0'

            # Formatar GB (coluna C) se existir
            if tipo == "TotalSizeMB":
